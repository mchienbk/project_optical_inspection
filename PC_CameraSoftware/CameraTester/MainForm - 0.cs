...
//---------------------------------------------------------------------------------------------------
        private void File_Inspecton()
        {
            ...
            string[] inputFilePath = Directory.GetFiles(@"D:\ImageInspection\20171216-1527", "*.png");

            testImage = new Bitmap(inputFilePath[0]);
            outImage = new Bitmap(312, 312);
        ...
        private void Camera_Inspection()
        {
            ...
            for (int col = 1; col <= 3; col = col + 1)       //25 coulumn
            {
                ...
                for (int row = 1; row <= 3; row = row + 1)       //10 row
                {
                    ...
                    GoY(125); while (!WaitForAData(Timeout)) ;
                }
                GoY(-375); while (!WaitForAData(Timeout)) ;
                GoX(125); while (!WaitForAData(Timeout)) ;
            }
